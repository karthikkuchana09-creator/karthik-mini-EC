...
export default function Phase10CErrorState({
  ...
  const resolvedTitle = title || (status === 401 ? 'Authentication Required'
  ...
  const Icon = ERROR_ICONS[status] || FiAlertCircle;
  ...
  const content = (
    <div className={`flex flex-col items-center justify-center py-12 px-4 ${className}`} role="alert">
      ...
      <div className="flex items-center gap-3">
        {onRetry && (
          <button onClick={onRetry} className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
