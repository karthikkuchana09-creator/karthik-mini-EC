import { FiBarChart2, FiCheckSquare, FiCheckCircle, FiFile } from 'react-icons/fi';
...
export default function ReportFilters({
  ...
  const showProject = reportType === 'tasks';
  const showTeam = reportType === 'tasks';
  const showStatus = reportType === 'projects' || reportType === 'tasks' || reportType === 'approvals';
  const showPriority = reportType === 'projects' || reportType === 'tasks';
  ...
  const priorityOptions = reportType === 'projects'
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200/70 shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100">
        <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Report Type</h3>
      ...
      <div className="p-5">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
          ...
          {showProject && (
            <div>
              ...
              <input
                ...
                onChange={(e) => onFilterChange('project_id', e.target.value ? Number(e.target.value) : undefined)}
          ...
          {showTeam && (
            <div>
              <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">Team</label>
              <input
                type="number"
                value={filters.team_id || ''}
                onChange={(e) => onFilterChange('team_id', e.target.value ? Number(e.target.value) : undefined)}
                placeholder="Team ID"
                className="w-full px-2.5 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300"
                disabled={loading}
              />
