...
export default function WorkspaceDocuments() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [ws, docs] = await Promise.all([
        ...
        taskDocumentsApi.getTaskDocuments(workspaceId),
      ]);
      setWorkspace(ws);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const uploaded = await taskDocumentsApi.uploadTaskDocument(workspaceId, formData);
  ...
  const NAV_TABS = [
    { label: 'Overview', path: `/workspaces/${workspace.id}` },
    { label: 'Messages', path: `/workspaces/${workspace.id}/messages` },
    { label: 'Tasks', path: `/workspaces/${workspace.id}/tasks` },
    { label: 'Documents', path: `/workspaces/${workspace.id}/documents` },
    { label: 'Members', path: `/workspaces/${workspace.id}/members` },
    { label: 'Channels', path: `/workspaces/${workspace.id}/channels` },
  ];

  return (
    <div className="page-container max-w-4xl">
      <button onClick={() => navigate(`/workspaces/${workspace.id}`)} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-600 transition-colors mb-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200/70 shadow-sm overflow-hidden">
        <div className="border-b border-gray-100 px-6 pt-4 pb-0">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-lg font-bold text-gray-900">{workspace.name} — Documents</h1>
            <div>
