...
export default function WorkspaceTaskDetails() {
  ...
  const fetchTask = useCallback(async () => {
    setLoading(true);
    try {
      const data = await workspaceTasksApi.getWorkspaceTask(wId, tId);
      setTask(data);
    } catch (err) {
      if (err?.response?.status === 404) {
        toast.error('Task not found');
        navigate(`/workspace-list/${wId}/tasks`);
      } else {
  ...
  useEffect(() => {
    fetchTask();
    fetchDocuments();
  }, [fetchTask, fetchDocuments]);

  const handleStatusChange = async (newStatus) => {
    setTask((prev) => ({ ...prev, status: newStatus }));
    try {
      const updated = await workspaceTasksApi.updateWorkspaceTaskStatus(wId, tId, newStatus);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await workspaceTasksApi.deleteWorkspaceTask(wId, tId);
    ...
    return (
      <div className="page-container max-w-4xl">
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <button onClick={fetchTask} className="btn-secondary text-sm">Try Again</button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200/70 shadow-sm overflow-hidden">
        <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-0">
          ...
          <div className="flex gap-1 overflow-x-auto scrollbar-none" aria-label="Tabs">
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 sm:px-5 py-3 rounded-t-xl text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${
                    isActive
        ...
        <div className="border-t border-gray-200/70 p-6 sm:p-8">
          {activeTab === 'overview' && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
              ...
              <div>
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Created By</h4>
                <p className="text-sm text-gray-900">{task.created_by_name || task.created_by || 'Unknown'}</p>
              </div>
              <div>
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Task ID</h4>
                <p className="text-sm text-gray-900 font-mono tabular-nums">#{task.id}</p>
              </div>
              <div>
